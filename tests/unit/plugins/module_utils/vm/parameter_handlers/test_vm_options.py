...
class TestVmOptionsParameterHandler:
    ...
    def test_init(self):
        ...
        assert obj._vm_option_params['encrypted_fault_tolerance'] == 'ftEncryptionOpportunistic'

    def test_verify_parameter_constraints(self, vm_options_parameter_handler):
        vm_options_parameter_handler._verify_parameter_constraints_virt_based_security = Mock()
        vm_options_parameter_handler._verify_parameter_constraints_enable_encryption = Mock()
        vm_options_parameter_handler.verify_parameter_constraints()
        assert vm_options_parameter_handler._verify_parameter_constraints_virt_based_security.call_count == 1
        assert vm_options_parameter_handler._verify_parameter_constraints_enable_encryption.call_count == 1

        vm_options_parameter_handler._vm_option_params = {'maximum_remote_console_sessions': 41}
        vm_options_parameter_handler.verify_parameter_constraints()
        assert vm_options_parameter_handler.error_handler.fail_with_parameter_error.call_count == 1
    ...
    def test_verify_parameter_constraints_virt_based_security(
        self, vm_options_parameter_handler, enable_vbs, firmware, secure_boot, enable_hardware_assisted_virtualization, vm, expected_error_count
    ):
        ...
        assert vm_options_parameter_handler.error_handler.fail_with_parameter_error.call_count == expected_error_count
    ...
    @patch('ansible_collections.vmware.vmware.plugins.module_utils.vm.parameter_handlers._vm_options.vim')
    def test_populate_config_spec_with_parameters(self, mock_vim, vm_options_parameter_handler):
        configspec = Mock()
        configspec.flags = None
        configspec.bootOptions = None
        vm_options_parameter_handler._vm_option_params = {"maximum_remote_console_sessions": 4}
        ...
        vm_options_parameter_handler._vm_option_params = {"enable_io_mmu": True, "enable_secure_boot": True}
        vm_options_parameter_handler.populate_config_spec_with_parameters(configspec)
        assert configspec.flags.vvtdEnabled is True
        assert configspec.bootOptions.efiSecureBootEnabled is True

    def test_compare_live_config_with_desired_config(self, vm_options_parameter_handler):
        vm_options_parameter_handler.compare_live_config_with_desired_config()
        assert (
            vm_options_parameter_handler.change_set.check_if_change_is_required.call_count > 1
        )
