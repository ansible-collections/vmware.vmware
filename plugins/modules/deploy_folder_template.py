...
class VmwareFolderTemplate(ModuleVmDeployBase):
    ...
    def resolve_template_object(self):
        ...
        if self.params['template_id']:
            ...
            folder = self.get_folders_by_name_or_moid(fq_folder_path, fail_on_missing=True)
            templates = self.get_objs_by_name_or_moid(
                [vim.VirtualMachine], self.params['template_name'],
                return_all=True, search_root_folder=folder
            )
            ...
            if not template:
                self.module.fail_json(msg=(
                    "Unable to find template with name %s in folder %s" %
                    (self.params['template_name'], fq_folder_path)
                ))
    ...
    def deploy(self, deploy_spec):
        ...
        try:
            task_result, _ = RunningTaskMonitor(task).wait_for_completion()   # pylint: disable=disallowed-name
