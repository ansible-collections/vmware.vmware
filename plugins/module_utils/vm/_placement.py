...
class VmPlacement(ModulePyvmomiBase):
    ...
    def get_datacenter(self, param='datacenter'):
        ...
        self._datacenter = self.get_datacenter_by_name_or_moid(self.params[param], fail_on_missing=True)
        return self._datacenter

    def get_datastore(self, datastore_param='datastore', datastore_cluster_param='datastore_cluster'):
        if self._datastore:
    ...
    def get_resource_pool(self, resource_pool_param='resource_pool', cluster_param='cluster'):
        ...
        if self.params.get(resource_pool_param):
            self._resource_pool = self.get_resource_pool_by_name_or_moid(
                self.params[resource_pool_param],
                fail_on_missing=True
            )
    ...
    def get_folder(self, folder_param='folder', datacenter_param='datacenter'):
        ...
        if not self.params.get(folder_param):
            fq_folder = format_folder_path_as_vm_fq_path('', self.params[datacenter_param])
        else:
            fq_folder = format_folder_path_as_vm_fq_path(self.params.get(folder_param), self.params[datacenter_param])
    ...
    def get_esxi_host(self, param='esxi_host'):
        ...
        self._esxi_host = self.get_esxi_host_by_name_or_moid(self.params[param], fail_on_missing=True)
        return self._esxi_host
