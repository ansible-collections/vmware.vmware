...
Skills live in `.agents/skills/` (YAML frontmatter + instructions). They may be stored in subdirectories or symlinks in `.agents/skills/**`. Recursively search for any files in those directories named `SKILL.md`. At session start, scan and register all skills in that directory recursively. When a request matches a skill's trigger, load and apply it.
