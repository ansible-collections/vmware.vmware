...
- The run-tests skill can be used, if one is defined. Sanity and linters should always be run and must pass. Unit tests should be run for functional changes.
- Python code should have corresponding unit tests with coverage of at least 50%. 70% is the preferred target.
- Integration tests cover both the happy path and idempotency (running the same task twice).
- Integration tests cover the `state: absent` path where applicable.
